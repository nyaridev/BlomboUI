...
def list_models() -> dict[str, list[dict[str, str | int]]]:
    root = models_root()
...
def list_kind(kind: str) -> list[dict[str, str | int]]:
    if kind == "wildcards":
        return _scan("wildcards", wildcards_root(), WILDCARD_EXTS)
    return _scan(kind, models_root() / kind, KINDS[kind])


def refresh_models(kind: str | None = None) -> dict[str, list[dict[str, str | int]]]:
...
def _scan(kind: str, folder: Path, exts: tuple[str, ...]) -> list[dict[str, str | int]]:
    ...
    model_meta.reconcile(kind, [str(item["path"]) for item in items])
...
def model_info(kind: str, rel: str) -> dict | None:
    ...
    return {
        ...
        "hashing": row is None,
        "types": model_meta.get_types(kind, posix),
