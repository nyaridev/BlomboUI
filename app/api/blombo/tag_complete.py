...
from blombo import autocomplete, db, model_meta, settings
...
def star_threshold(unique: int | None = None) -> int:
    if unique is None:
        row = db.query_one("SELECT COUNT(*) AS n FROM prompt_tags")
        unique = int(row["n"]) if row else 0
...
def _record(prompt: str, negative: str, extra: list[str]) -> None:
    row = db.query_one("SELECT prompt, negative FROM prompt_tag_state WHERE id = 1")
    if row and str(row["prompt"]) == prompt and str(row["negative"]) == negative:
        return
    tags: list[str] = []
    seen: set[str] = set()
...
def frequent() -> dict[str, Any]:
    try:
        unique_row = db.query_one("SELECT COUNT(*) AS n FROM prompt_tags")
        unique = int(unique_row["n"]) if unique_row else 0
        rows = db.query(
