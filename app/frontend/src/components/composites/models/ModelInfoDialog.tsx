...
import { ModelInfoActions, ModelInfoBody, ModelInfoHeader } from '@/components/composites/models/ModelInfoLayouts.tsx'
import { TilePreview } from '@/components/composites/models/TilePreview.tsx'
import { clampLora, loraRange, modelFileName } from '@/components/composites/models/modelInfoLayouts.ts'
...
export function ModelInfoDialog({
  ...
  function save() {
    ...
    void (async () => {
      ...
      if (pending === 'clear') {
        const tick = await deleteModelThumb(kind, item.path, saveThumbView(scopeKey))
        setThumb(tick)
        setPending(null)
        ...
        const dest = pendingMeta.origin === 'civitai' ? civitaiSaveThumbView(kind) : saveThumbView(scopeKey)
  ...
  return (
    <Dialog
      ...
      className={[
        ...
        'max-h-[min(92vh,calc(100vh-2rem))]',
        wide ? 'w-[min(96vw,80rem)]' : 'w-[min(92vw,42rem)]',
      ].join(' ')}
    >
      <ModelInfoHeader
