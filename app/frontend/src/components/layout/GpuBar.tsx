...
export function GpuBar() {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <IconButton className="text-ink" aria-label="Unload models"
        title="Unload models"
        disabled={busy}
        onClick={() => void run(true, false)}
      >
        <AppIcon id="download" /></IconButton>
      <IconButton className="text-ink" aria-label="Free model and node cache"
        title="Free model and node cache"
        disabled={busy}
        onClick={() => void run(false, true)}
      >
        <AppIcon id="database" /></IconButton>
    </div>
  )
}
