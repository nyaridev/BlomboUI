...
export function DownloadsView({
  ...
  function openCivitai(target: CivitaiMenu, local: boolean) {
    if (local) {
      openInCivitaiPanel({ id: target.modelId, name: target.name }, target.versionId)
      navigate('/models')
      return
    }
    window.open(
                      ...
                      <div className="flex flex-col gap-1 border-t border-line px-2 py-2">
                        {group.files.map((row) => {
                          ...
                          >
                            ...
                            {row.status === 'failed' ? (
                              <IconButton aria-label="Retry download"
                            ...
                            <IconButton aria-label="Open in Explorer"
                            ...
                            <IconButton aria-label="Remove from list"
                              ...
                              onClick={() =>(row.status === 'done' || row.status === 'failed') && setRemoveTarget(row.item)
                              }
                            >
