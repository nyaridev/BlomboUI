...
import { getPromptTagUsage, suggestPromptTags, type FrequentPromptTag, type ModelEntry, type PromptTagHit, type ThumbView } from '@/lib/api.ts'
...
import { PromptHighlight } from '@/views/generate/panels/chrome/sections/prompt/PromptHighlight.tsx'
...
function suggestThumb(
  ...
  return modelThumbSrc(kind, item, hit.kind === 'lora' ? loraView : wildView)
}
...
export function PromptField({
  ...
  checkpoint?: string
  companionNegative?: string
  onCompanionNegative?: (value: string) => void
}) {
  const storeCheckpoint = useGenerateStore((s) => s.checkpoint)
  const storeNegative = useGenerateStore((s) => s.negativePrompt)
  const setStoreNegative = useGenerateStore((s) => s.setNegativePrompt)
  const checkpoint = checkpointOverride ?? storeCheckpoint
  const negativePrompt = companionNegative ?? storeNegative
  const setNegativePrompt = onCompanionNegative ?? setStoreNegative
          ...
          {showThumb && active ? (
            ...
            >
              <div style={{ width: thumbW }}>
                <TilePreview
                  src={suggestThumb(active, loras, wildcards, loraView, wildView)}
                  mark=""
                  eager
