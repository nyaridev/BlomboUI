...
function HoverIconButton({
  ...
  return (
    <button
      type="button"
      className={[
        'flex shrink-0 items-center justify-center overflow-hidden rounded text-muted',
        solid
          ? 'aspect-square h-5 w-5 bg-bg/70 hover:text-ink hover:scale-110'
          : 'aspect-square h-5 w-0 transition-[width] duration-200 ease-out group-hover:w-5',
        hover === 'red' ? 'hover:bg-red hover:text-ink' : 'hover:bg-line hover:text-ink',
      ].join(' ')}
      aria-label={label}
      title={label}
      onClick={(event) => {
    ...
    >
      <span className="scale-75">{children}</span>
    </button>
  )
}
...
export function ModelTile({
  ...
  if (spec.text) {
    return (
      <div className="relative shrink-0" data-model-tile="">
        {dropMarker}
        <div
          className={[
        ...
        >
          ...
          {onEdit ? (
            <HoverIconButton label={`Change ${role}`} hover="line" onClick={onEdit}>
              <AppIcon id="pencil" size={12} />
            </HoverIconButton>
          ) : null}
          {onClear ? (
            <HoverIconButton label={`Clear ${role}`} hover="red" onClick={onClear}>
    ...
    <div className="relative shrink-0" data-model-tile="">
      ...
      >
        <button
          type="button"
          className={[
            'relative block transition duration-150 ease-out group-hover:brightness-110',
            active ? TILE_GLOW : '',
            dimmed ? 'opacity-40' : '',
          ].join(' ')}
          title={title}
        ...
        >
          ...
          {empty ? (
            <span className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center text-muted">
              <AppIcon id="plus" size={style === 'compact' ? 18 : 22} />
            </span>
          ) : null}
        </button>
        {onEdit || onClear ? (
          <div className="pointer-events-none absolute top-1 right-1 z-20 flex gap-0.5 opacity-0 scale-75 transition duration-150 ease-out group-hover:pointer-events-auto group-hover:scale-100 group-hover:opacity-100">
            {onEdit ? (
              <HoverIconButton label={`Change ${role}`} hover="line" solid onClick={onEdit}>
                <AppIcon id="pencil" size={12} />
              </HoverIconButton>
            ) : null}
            {onClear ? (
              <HoverIconButton label={`Clear ${role}`} hover="red" solid onClick={onClear}>
                <AppIcon id="x" size={12} />
              </HoverIconButton>
            ) : null}
          </div>
