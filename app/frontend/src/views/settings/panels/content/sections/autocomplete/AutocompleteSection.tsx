    ...
    <div className="flex flex-col gap-2 py-3">
      <div className="flex items-center gap-2">
        <label className="flex min-w-0 flex-1 items-center gap-2 text-sm text-ink">
          <CheckboxControl checked={rule.enabled} onChange={(checked) => setAutocompleteList(name, { enabled: checked })} />
          <span className="truncate">{name}</span>
        </label>
        {size > 0 ? <span className="shrink-0 text-xs text-muted">{formatSize(size)}</span> : null}
        <SettingsReset setting="autocompleteLists" field={name} />
      </div>
...
export function AutocompleteBehaviorSection({ query = '' }: { query?: string }) {
  ...
  return (
    <div className="flex max-w-xl flex-col gap-3">
      <SettingsCard query={query} title="Tags" terms="autocomplete enable disable exclude include whitelist blacklist types">
        <SettingsField setting="autocompleteEnabled">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={autocompleteEnabled} onChange={setAutocompleteEnabled} />
            Enable autocomplete
          </label>
        </SettingsField>
        <div className={autocompleteEnabled ? 'flex flex-col gap-2' : 'pointer-events-none flex flex-col gap-2 opacity-40'}>
          <SettingsField setting="autocompleteMode">
            <div className="flex gap-1">
              {(['exclude', 'include'] as AutocompleteMode[]).map((mode) => (
                <button
                  key={mode}
          ...
          <SettingsField setting="autocompleteTypes">
            <ChipSelect
              options={pickerOptions}
      ...
      <SettingsCard query={query} title="Wildcards" terms="wildcard enable autocomplete">
        <SettingsField setting="wildcardCompleteEnabled">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={wildcardCompleteEnabled} onChange={setWildcardCompleteEnabled} />
            Enable wildcard autocomplete
          </label>
        </SettingsField>
      </SettingsCard>
      <SettingsCard query={query} title="LoRA" terms="lora enable autocomplete trigger words">
        <SettingsField setting="loraCompleteEnabled">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={loraCompleteEnabled} onChange={setLoraCompleteEnabled} />
            Enable LoRA autocomplete
          </label>
        </SettingsField>
        <SettingsField setting="loraTriggerCompleteEnabled">
          <label
            className={[
          ...
          >
            <CheckboxControl checked={loraTriggerCompleteEnabled} onChange={setLoraTriggerCompleteEnabled} disabled={!loraCompleteEnabled} />
      ...
      <SettingsCard query={query} title="Thumbnails" terms="thumbnail thumb preview tile scale size wildcard lora">
        <SettingsField setting="wildcardCompleteThumbs">
          <label
            className={[
              'flex items-center gap-2 text-sm text-ink',
          ...
          >
            <CheckboxControl checked={wildcardCompleteThumbs} onChange={setWildcardCompleteThumbs} disabled={!wildcardCompleteEnabled} />
            Show wildcard thumbnails
          </label>
        </SettingsField>
        <SettingsField setting="loraCompleteThumbs">
          <label
            className={[
              'flex items-center gap-2 text-sm text-ink',
              loraCompleteEnabled ? '' : 'pointer-events-none opacity-40',
          ...
          >
            <CheckboxControl checked={loraCompleteThumbs} onChange={setLoraCompleteThumbs} disabled={!loraCompleteEnabled} />
            Show LoRA thumbnails
          </label>
        </SettingsField>
        <SettingsField setting="autocompleteThumbScale" className="flex items-start gap-2">
      ...
      <SettingsCard query={query} title="Frequent tags" terms="frequent tags enable star usage">
        <SettingsField setting="frequentTagsEnabled">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={frequentTagsEnabled} onChange={setFrequentTagsEnabled} />
...
export function AutocompleteSection({ query = '' }: { query?: string }) {
