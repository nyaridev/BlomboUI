...
export function LabelsSection({ query = '' }: { query?: string }) {
          ...
          {orderedNames(marks).map((name) => {
            ...
            return (
              <div key={name} className="flex items-center gap-1.5 rounded-md border border-line bg-panel p-1.5">
                ...
                <SettingsReset setting="civitaiMarks" field={name} />
                <IconPicker
                  value={row.icon ?? null}
                  colors={[]}
                ...
                <span className="flex w-8 shrink-0 justify-center">
                  {row.icon ? (
                    <IconButton className="shrink-0" aria-label={`Clear icon for ${name}`}
                      title="Clear icon"
