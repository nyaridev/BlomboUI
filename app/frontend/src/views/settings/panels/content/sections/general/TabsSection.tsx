...
import { GENERATE_TABS, HIDEABLE_GENERATE_TABS, type GenerateTab } from '@/views/generate/panels/workspace/tabs.ts'
...
export const TABS_QUERY =
  'tabs exclude hide order reorder generate main keybind shortcut ctrl alt 1 2 3 4 5 6 7 8 9 errors history settings file info gallery models wildcard manager scopes'

export function TabsSection({ query = '' }: { query?: string }) {
  ...
  return (
    <div className="flex max-w-xl flex-col gap-3">
      ...
      >
        <SettingsBlock query={query} title="Exclude" terms="hide generate file info gallery models wildcard manager scopes errors" setting="hiddenMainTabs">
          <ChipSelect
          ...
          />
          <p className="text-xs text-muted">History and Settings stay visible. Selected tabs leave the header.</p>
        </SettingsBlock>
        <SettingsBlock query={query} title="Order" terms="reorder left generate file info gallery models wildcard manager scopes" setting="mainTabOrder">
          <ChipSelect
            mode="order"
          ...
          />
          <p className="text-xs text-muted">Drag to reorder the left header tabs. Errors, History, and Settings stay on the right.</p>
        </SettingsBlock>
      </SettingsCard>
      <SettingsCard
        query={query}
      ...
      >
        <SettingsBlock query={query} title="Exclude" terms="hidden tabs" setting="hiddenGenerateTabs">
          <ChipSelect
        ...
        <SettingsBlock query={query} title="Order" terms="reorder generation base model lora wildcards other" setting="generateTabOrder">
          <ChipSelect
            mode="order"
      ...
      <SettingsCard query={query} title="Tab shortcuts" terms="keybind shortcut ctrl alt follow layout 1 2 3 4 5 6 7 8 9">
        <SettingsField setting="mainTabKeysFollowLayout">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={mainTabKeysFollowLayout} onChange={setMainTabKeysFollowLayout} />
            Ctrl+1…9 follow main tab order and exclusions
          </label>
        </SettingsField>
        <SettingsField setting="generateTabKeysFollowLayout">
          <label className="flex items-center gap-2 text-sm text-ink">
            <CheckboxControl checked={generateTabKeysFollowLayout} onChange={setGenerateTabKeysFollowLayout} />
