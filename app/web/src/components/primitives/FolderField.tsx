...
const FIELD =
  'box-border h-8 min-w-0 flex-1 rounded border border-line bg-field px-2 py-0 font-mono text-sm leading-[1.875rem] text-ink outline-none placeholder:text-muted focus:border-accent disabled:opacity-70'
...
export function FolderField({
  ...
  return (
    <div className="flex h-8 min-w-0 flex-1 items-center gap-1">
      ...
      />
      {readOnly ? null : (
        <button type="button" className="icon-btn shrink-0" aria-label="Browse folder" onClick={() => void browse()}>
          <AppIcon id="folder" />
        </button>
      )}
      <button
