...
export const GRIDS_QUERY =
  'grids batch grid contact sheet jpg jpeg quality max images row count empty gaps fill'
  ...
  return (
    <div className="flex max-w-xl flex-col gap-6">
      <SettingsBlock
        query={query}
        title="Batch grid"
        terms="contact sheet jpg jpeg save show first"
      >
        <label className="flex items-center gap-2 text-sm text-ink">
          ...
          />
          Save a JPG contact sheet and show it first
        </label>
      </SettingsBlock>
      ...
      <SettingsBlock query={query} title="Empty gaps" terms="prevent fill long line">
        ...
        <p className="text-xs text-muted">
          Off keeps a near-square sheet (11 shots can be 4×3 with a hole). On packs without holes — 11
          shots become a single line.
        </p>
      </SettingsBlock>
    </div>
  )
